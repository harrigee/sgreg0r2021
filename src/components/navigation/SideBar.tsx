export function SideBar({ items }: { items: { uid: string, displayName: string, isOnline: boolean }[] }) {
  return <div className="w-8 bg-green-400 transition-all ease-in-out duration-500 hover:w-80 hover:bg-zinc-800 pt-8 overflow-y-scroll">
    <div>
      {items.map((item, i) =>
        <div
          ...
          <p className="text-white text-left self-center ml-4">
            {item.displayName}
          </p>
        </div>
      )}
